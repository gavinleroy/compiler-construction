...
object L3Parser {
  ...
  private class L(indexToPosition: Int => Position) {
    ...
    private def integer10[_: P] = P((sign.? ~ digit10 ~/ digit10.rep).!.map {
    ...
    def identStr[_: P] = P((identStart ~/ identCont.rep ~/ identSuffix.?).!)
    def identifier[_: P] = P((Index ~ identStr).map { case (i, n) =>
      Ident(n)(i)
    })
  ...
  private class S(indexToPosition: Int => Position) {
    ...
    private def bindings[_: P] = P(par(binding.rep))
  ...
  private def sFun(args: Seq[String], body: Tree)(implicit
      p: Position
  ): Tree = {
    var f = freshName("fun")
    LetRec(Seq(Fun(f, args, body)), Ident(f))
  }

  private def sLet_*(bdgs: Seq[(String, Tree)], body: Tree)(implicit
      p: Position
  ): Tree = bdgs.foldRight(body) { case (bndg, a) => Let(Seq(bndg), a) }

  private def sBegin(exprs: Seq[Tree])(implicit p: Position): Tree =
    exprs.reduceRight { (l, a) => Let(Seq((freshName("temp"), l)), a) }

  private def sRec(name: String, bdgs: Seq[(String, Tree)], body: Tree)(implicit
      p: Position
  ) = LetRec(
    Seq(Fun(name, bdgs.map(_._1), body)),
    App(Ident(name), bdgs.map(_._2))
  )

  private def sAnd(es: Seq[Tree])(implicit p: Position): Tree =
    es.reduceRight { (l, a) => If(l, a, Lit(BooleanLit(false))) }

  private def sOr(es: Seq[Tree])(implicit p: Position): Tree =
    es.reduceRight { (l, a) =>
      {
        var v = freshName("temp")
        Let(Seq((v, l)), If(Ident(v), Ident(v), a))
      }
    }
  ...
  private def sCond(clses: Seq[(Tree, Seq[Tree])])(implicit p: Position): Tree =
    clses.foldRight(Lit(UnitLit): Tree) { case ((e, stmts), a) =>
      If(e, sBegin(stmts), a)
    }
    ...
    Let(
      Seq(
        (
          b,
          Prim(
            "block-alloc-" + BlockTag.String.id,
