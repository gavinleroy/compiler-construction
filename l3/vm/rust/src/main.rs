...
fn load(file_name: &str, engine: &mut Engine) -> Result<(), String> {
    let file = File::open(file_name).map_err(|_| format!("can't open file {}", file_name))?;

    for (index, line) in BufReader::new(file).lines().enumerate() {
        let line = line.map_err(|_| format!("I/O problem at line {}", index))?;
